...
| `@roomba` | `<item name>` | one line per matching item — total quantity across every gang-house room it was seen in during a Roomba sweep, and the room locators (e.g. `total: 5x rope and grapple - seen in 15/12, 15/13`) — a loose query matching several similarly-named items (e.g. "head" matching every "severed head of ___") gets one line each, capped at 5 with an overflow tail — or "no record" when nothing matches at all; own permission ("Query item location"). Silent unless **Enable @roomba responses** is checked on the Roomba tab — see Roomba (Player Workshop) below |
| `@roomba sync` | — | (client-to-client) replies with your entire item-sighting log, compressed, so the requester's client merges it straight in — no file, no Discord, no import/export; see Roomba (Player Workshop) below; same `@roomba` permission |
...
**Shared per BBS, not per character.** Room labels, the hidden-search settings, the **Enable @roomba responses** toggle, and the item-location log below are all saved against the BBS you're connected to, not your character — a BBS ties to one game-data set and every character on it shares the same gang house, so labeling rooms (or running a sweep) on any one character makes them available to every other character on that board.
...
**Master List** — a separate button that opens a full, **sortable** table (click any column header — Item, Qty, Seen In, Market) of everything the item-location log currently knows: one row per item per room it was seen in (quantity included), plus a **Market** column cross-referencing that item's `Obtained From` shop data — every shop that buys or sells it, priced at a fixed 50 charm (MajorMUD's neutral "retail" point), **excluding any shop that sits inside one of this gang house's own labeled rooms** — you don't need a reminder that your own stash room "sells" what you just put there. An item with no market outside the gang house reads "(no outside market)". Updates live as new scans (sweep or Inventory-only) come in, same as `@roomba`'s log — they're the same data.

**Item-location log + `@roomba`.** Every room floor Roomba observes during a scan (recon, an Inventory-only lap, or the post-sort final scan) is recorded as that room's known contents. A gang house can stock the same item in more than one room at once, so the log tracks sightings per room, not just one "last seen" spot per item — re-scanning a room updates only that room's own entries (an item no longer on its floor drops off, without touching that same item's sighting anywhere else). Tick **Enable @roomba responses** on the tab to let a gangpath'd `@roomba <item name>` (from a gang member you've granted the "Query item location" remote-control permission on the Players tab) get back **one reply line per matching item** — the total quantity summed across every room currently holding it, followed by the room locators (map/room, capped at 10 with a "+N more" tail for anything scattered wider than that) — instead of a separate line per room, which used to flood the channel for anything stocked in several places. A loose query that matches several distinct items at once (item names often share words — "severed head of goru-nezar" and "severed head of darksong" both match "head") gets a line for each, capped at 5 with its own overflow tail, rather than refusing to answer just because more than one name matched. Left unticked, the client never replies to `@roomba` at all. Like the room labels, this log and the toggle are shared BBS-wide.

**`@roomba sync`** — the no-hassle way to hand your item-location log to a gang member starting fresh on their own MudPlay install, no file/Discord/import-export needed. They gangpath `@roomba sync`; your client (with **Enable @roomba responses** on and them granted "Query item location") replies with your whole log, compressed onto a couple of chat lines, and it merges straight into theirs. Unlike `@timer sync`'s boss-timer merge, there's no review window: a room-contents sighting has no "conflict" to weigh the way a disputed kill time does, so whichever side saw an item more recently just wins, silently. You don't need to send `@roomba sync` yourself to receive one — any client with responses enabled adopts a sync reply the moment it arrives.
