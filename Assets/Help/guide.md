...
**Character bar** — a strip across the top (once a character is loaded) showing your name/level/class, live HP, live Mana or Kai (whichever your class uses), your currently-equipped weapon's HitMagic, how many attack spells you've obtained, and your **AC vs Evil** (your Armour Class plus the flat +10 Shadow bonus if any worn item carries it, plus your worn Prot Evil — the combined defense an evil monster's attack actually rolls against). It's **editable** — click into it and type a correction if it doesn't match your in-game `@st` reading (the auto-calc can still miss an AC-boosting spell buff MudPlay isn't tracking); your edit also feeds Hits You % for evil monsters, not just this display number, and it sticks until you close and reopen the window. Like every AC-based figure in this window (AC vs Evil and each row's Hits You %), it **assumes your configured self-buffs are up** — the AC your buffs grant is folded into your Armour Class first, so the numbers reflect how you'll actually fight, not how exposed you are standing around unbuffed. It updates live as HP/mana tick and stays current if you swap gear or learn a new spell while the window is open.

The left list is filterable by name and shows six columns — **Name**, **HP**, **EXP**, **Accuracy** (the monster's own physical-attack accuracy — the raw number that feeds into Hits You %, so you can see *why* it hits at the percent shown, not just the outcome; blank for a spell-only monster with no physical attack), **Hits You %** (that monster's own attack's chance to land on you, given your live AC/Dodge, your Shadow bonus if you have one, and whichever ward — Prot Evil/Prot Good — applies to its alignment), and **Est. Rounds to Kill** (rounds for your currently-equipped weapon's Normal attack to drop it, at your live accuracy/damage/swings/crit — the same projection the Character Workshop's Calculators tab uses, shown here as "—" when you're unarmed or can't out-damage it). A monster that would take longer than the **rounds-to-kill cap** (a spinner next to the Hits-You-% checkboxes, default 999, editable right in this window — no need to open Settings) is **hidden from the list entirely** — a superboss can otherwise project into the millions of rounds, and the cap is a triage tool (show me what I can kill fast enough), not just cosmetic number-shortening. A monster you can't kill at all with your current weapon still shows as "—", since "not killable" is different information from "too slow." Changes save automatically per character. Every column is independently sortable (click a header; click again to reverse). **Once a character is loaded, a monster with no computable Hits You % (an NPC/caster-only record with no catalogued physical attack — a trainer, quest-giver, etc.) is dropped from the list entirely** — it isn't a meaningful "can this thing hurt me" entry, so it's never just left blank.

**Recommend a mob** (top of the list, next to the search box — once a character is loaded) picks the best fight from whatever's *currently shown* below, respecting your own name filter, Hits-You-% bands, and rounds-to-kill cap rather than second-guessing them. It scores by EXP earned per round of fighting, discounted by the monster's own chance to hit back — a monster you can barely land a hit on already scores near the bottom, since that's baked into a high (or "not killable") Est. Rounds to Kill. This is a first-pass heuristic and expected to get tuned over time; clicking it selects the pick so its detail panel opens on the right.
