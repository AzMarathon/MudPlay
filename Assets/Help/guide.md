...
**Left-click any room** to inspect it in the **ROOM INFO** rail panel (it pops open at the top of the rail when you click, unless you're mid loop- or lair-build, where it updates quietly instead of interrupting). It lists clickable links to everything attached to that room — the **room name** itself (click it to open the room's record), each **monster** that lairs, is summoned, or is placed there, each **floor item** the room scatter-places (via its `roomitem` command), the **shop** as a whole when the room hosts one, and the room's cast-on-enter **room spell**. Clicking a **monster** opens its full record in a **monster dialog** (the same "Other Info" record you get from the Monsters browser tab); the room / item / shop / spell links open that record in the **Game Data Browser**. Either way it's a quick jump from "what's in this room" to the full record without hunting through the browser's tables.
