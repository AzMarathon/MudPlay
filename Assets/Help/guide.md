...
A **drag bar** sits between the config table and the timer bars — grab it to re-divide the space between the two. Where the config table sits relative to the bars — **above / below / left / right** — is set on **Settings → General → "Buff Watchdog layout"**; changing it reflows an open Buff Watchdog at once. Both panels grow to fill the window, so there's no wasted space whichever way you arrange them.
