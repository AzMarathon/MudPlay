...
**Running a sweep:** open the **Player Workshop → GH Management** tab to review your labeled destinations (each row summarizes its rules, with "[catch-all]" flagging the fallback room) and click **Start Sweep**. Recon laps are configurable here (default 2). The **Search rooms for hidden items** checkbox is **off by default** — Roomba sorts only what's plainly visible on the floor; tick it to also send `sea` in each room during recon and sort what's hidden (its **Searches per room** count, default 3, applies only then, and greys out while the box is unticked). The checkbox is saved per character. The sweep runs in two phases:
- **Recon:** the expanded circuit is walked, including unlabeled rooms between destinations, purely observing — nothing is picked up or moved yet. With hidden-item search on, each room is also `sea`'d; visible floor items and hidden search results are combined without counting the same rediscovered stack more than once. With it off, only the visible floor is observed and no `sea` goes out.
- **Sorting:** visible items are picked up immediately with no more searches after recon. When hidden search was on, an item found only by recon's `sea` is logged as `(hidden)` and only that item causes the configured searches to run again before `get`. Roomba waits for the game's get/drop confirmations, requests a fresh `i`, and verifies inventory before movement resumes. It then routes directly to the nearest carried destination; with nothing carried, it targets the nearest remaining source instead of completing the old one-way circuit. An item matching no rule anywhere goes to the catch-all room if one is designated; otherwise it's left in place rather than guessed at.
...
**Important notes:** Roomba Mode refuses to start while another movement engine (a manual walk, a Loop, or Auto-Lair) is already active, and while running it behaves like any other Loop for the toolbar Pause/Stop buttons and the manual-move-pauses-navigation rule. Roomba attempts every queued pickup even when it may make you Heavy, and a no-progress lap does not end the sweep: failed or unverified pickups remain queued (and, with hidden search on, hidden items are re-searched) until every move is delivered or you stop Roomba manually. Gang-house guard emblems (items named like "Gold Emblem" — the ones that keep that house's guards from attacking you) are never swept as clutter, and a sweep only ever acts on items it found on a circuit-room floor during its own recon — never anything already in your pack.
