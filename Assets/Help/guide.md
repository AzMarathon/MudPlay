...
- **Folders** — click **New folder** to add your own named submenu that flies out to the side. To fill it: select the folder and add items from the pool (the **Add** button reads **Add into folder** while a folder is selected). To move an item that's *already* in the menu, select it and use **Into folder →** (tuck it into the folder directly above) or **← Out of folder** (pop it back to the top level). Reordering a folder moves its contents with it.
