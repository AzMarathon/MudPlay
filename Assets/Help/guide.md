...
| `@roomba` | `<item name>` | one line — total quantity across every gang-house room that item was seen in during a Roomba sweep, and the room locators (e.g. `total: 5x rope and grapple - seen in 15/12, 15/13`) — or "no record"; own permission ("Query item location"). Silent unless **Enable @roomba responses** is checked on the Roomba tab — see Roomba (Player Workshop) below |
