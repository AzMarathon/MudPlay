...
**Seeing the route you're already on.** Once a route is *running*, a **Details…** button appears on the Navigation header (next to Go to…). It opens that same full step plan for whatever's currently executing — a point-to-point walk, a loop circuit, or an Auto-Lair approach — so you can check the path ahead without re-planning it. It adds one thing the picker's list doesn't: at every room on the route that has a **lair**, the lair's monsters are listed right under that step, each a **clickable link** that opens the monster's Game Data record — handy for sizing up what a hunting loop is about to walk into.
