...
A **Hits You %** filter dropdown narrows the list by how dangerous a monster's own attack is. It offers a set of contiguous %-bands — each its own discrete range, together covering the realm's whole range with no gap — and you tick **any combination** (multi-select): a monster shows if its Hits You % falls in **any** ticked band (tick the safe end and the risky end to see both while hiding the middle); tick none and every monster shows. The bands are **realm-dependent**, because the lowest a monster's attack can ever land differs — **2% on ParaMUD, 8% on Stock** — so ParaMUD offers `≤2%, 3–5%, 6–10%, 11–15%, …` while Stock skips the impossible 2% / 5% bands and starts at `≤8%`. The dropdown button shows how many bands you've picked, so the filter fits the realm you're actually playing.
...
- **The Roomba Log has an "Out of space" section**, and the map marks it. The log leads with what to do — the rooms that could have taken an item, confirmation they're all full, and the items affected — then lists every room that refused a drop. On the Navigation map, a Roomba room that's out of space gets an **amber ring** round its robot marker, so "which of my rooms are full" is a glance rather than a log read.
- **The log tells you which category ran out of space.** When an item has nowhere to go, the end-of-sweep summary names the *rooms that could have taken it* and confirms they're all full — so the fix is obvious: label another room for that category (or flag another catch-all). A flat list of items that went nowhere doesn't tell you what to do about it; this does.
