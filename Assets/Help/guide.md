...
**Add all blesses** bulk-adds every self-castable buff you've learned in one click — no dialog, since there's nothing to choose. It looks at your level and spellbook for every buff you can cast on yourself (self-only spells, and single-target spells aimed at you), skips anything already slotted, and where two of those buffs would strip each other off (e.g. **zeal** and **greater zeal**) it keeps only the higher-level one — so you never end up with both fighting over the same slot. It also skips anything that conflicts with a buff you've already configured by hand, self or party-wide, so it never clobbers your existing setup. It's alignment-aware where it can be: a class's spellbook often carries both sides of a holy/unholy pair you've learned over your career (only one of which you can actually cast right now), and once MudPlay has seen your alignment in a `who` this run, only the side you can currently cast is offered. It's self-only: a whole-party buff affects other players too, so those still go through **＋ Add buff** where you choose the targeting yourself. The button disables once there's nothing left to add.
