...
- A buff that's **actually up but isn't configured** — one you cast by hand, or a slot you just removed — still shows its live timer bar as a plain read-only entry, so clearing your config never hides a buff that's genuinely running. It simply won't be recast once it wears off.
