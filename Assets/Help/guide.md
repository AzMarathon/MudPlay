...
- A **whole-party** spell shows a **Party Wide** on/off toggle (cast it party-wide while you're in a party — one cast covers everyone) plus a **Solo** checkbox. Tick **Solo** (on by default) to also cast it when you're alone — a whole-party cast still lands on a lone character (a party of one), so it isn't wasted; untick it to make that buff party-only. A **self-only** spell shows just the **Self** box (which already fires solo or partied).
...
**Add all blesses** adds a row for every buff your CLASS can ever cast — on yourself or the whole party — in one click, no dialog, since there's nothing to choose. That's every self-only spell, every single-target spell aimed at you, and every whole-party spell your class's spell list contains, whether you've actually trained it yet or not, and **regardless of level** — a browsable roster for theorycrafting a build, not just what you can cast this exact moment. It also includes any **cast-on-use item** whose effect always lands on you or the whole party without needing to be aimed (a wielded weapon or staff like a bless-casting crozier, or an unlimited-use party item) the same way — an item whose effect needs a target isn't included, since `use <item>` can't be pointed at a specific person. Item-cast slots are broken out under their own **------- Weapons -------** heading, below the ordinary spell rows, so a growing spellbook doesn't bury the handful of weapon procs among dozens of learned spells. A row for a buff you haven't learned yet shows an **unlearned** chip next to its name — you can still check it to plan ahead, it just won't actually fire until you've trained it. It's alignment-aware where it can be: a class's spell list often carries both sides of a holy/unholy pair (only one of which you can actually cast, depending which way you're aligned), and once MudPlay has seen your alignment in a `who` this run, only the side you can currently cast is offered — a "non-evil only" spell is hidden from an evil character, while a "non-good only" one is offered to them same as a neutral character. Skips anything already slotted.

The rows land sorted **by level requirement, low to high** (ties broken alphabetically) — not by name — so a high-level pick shows up near the bottom with the other high-level buffs instead of wherever its name happens to sort. It doesn't just check off one pick per buff family and hide the rest — **every** eligible buff gets a row, so you can see (and switch to) any of them. Where two buffs would strip each other off (e.g. **zeal** and **greater zeal**), only the higher-level *learned* one comes pre-checked — a family member you haven't trained never comes pre-checked even if it'd otherwise outrank the others, since it wouldn't actually cast; its family members are still listed, just unticked, so you can swap to any of them by hand once you've learned them. Checking one member of a conflicting pair automatically **unticks the other** — and vice versa — so you never end up with both fighting over the same slot; this live swap applies to any row in the list, not just ones this button added. It also leaves alone anything that conflicts with a buff you've already configured by hand (self or party-wide) — it lists it, but won't pre-check it, so it never silently clobbers your existing setup. **Whole-party buffs are listed too** (chant and the like), sorted into the same level-ordered list — but they never come pre-checked, full stop: a whole-party cast affects other players, so that's always your call to make by ticking the row's own **Party Wide** box yourself, never something a bulk-add button decides for you. The button disables once there's nothing left to add.

**Remove all** clears the entire list in one click — every self, party, and whole-party slot, gone. If you've turned on **Confirm deletes** (Settings → BBS + Display → "Show confirmations" — off by default), it asks you to confirm first, the same prompt every other list delete in the app uses. Disabled when the list is already empty.
