...
**Find Best searches whatever the results grid currently shows** — not the whole catalog. Leave every filter at its default and it searches everything; narrow the grid first and it searches only that. This is deliberate: a plate-capable class's "best AC" is plate almost by construction (nothing else comes close on raw AC), so without a way to narrow the search there'd be no way to ask for anything more specific. Want the best AC available in **Leather** even though your class could wear Plate? Set **Armour Type** to Leather, pick **Armour Class** in the Find Best dropdown, and click it — only leather pieces are considered. The same applies to Slot, Weapon Type, Backstab-only, the name filter, and every stat-threshold filter.
