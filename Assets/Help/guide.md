...
**Searching for a counter en route.** When a route crosses a hazard you'd counter, the picker also offers a **"Search en route"** card. Pick it and the walker heads toward the hazard **searching each room on the way** (`sea`); if a counter turns up on the floor it's grabbed automatically and you cross, and if nothing turns up you stop at the hazard's edge — nothing bought, nothing lost. It's the way to source a counter that isn't sold anywhere but can be *found* (a hidden rope by the gypsy spirit, say) rather than buying one. (A searched-up counter is now collected by the route's own obtain pipeline, so it no longer depends on the Auto-Get engine being on and the item flagged auto-collect.)
