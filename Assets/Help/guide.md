...
- **Bosses** — a respawn-timer tracker. **Mark** or **Now** stamps a boss's kill time and the **100%** column counts down to its respawn; **Manage Bosses…** edits the list, and you can **Import / Export** a shared table. Tick **Stop before** to halt automation ahead of a boss. **Chest Offload…** opens a helper for cashing in boss chests: it snapshots your carried inventory and coin, lists the containers you're holding (click one to `open` it), then — after re-reading your inventory — shows the coin the chest gave and every new item, grouped into the fewest shops. Each item has an editable sell quantity (keep some, sell the rest) priced at a **charm** picker, plus a **Drop** button to bin the whole stack; a **Total to sell** figure sums everything selected across all shops. Each shop header shows a running total and **walks you there** when clicked — with the shop's map/room and the number of steps to reach it from your current room — and carries a **Sell** button that fires the sell commands (batched on Paradigm) once you're standing in the shop plus a **Drop All** button to bin that shop's items. The shops are ordered into a short nearest-first trip using the same routing the walker uses (respecting avoid rooms, usable teleport gates, and item/hazard/boat gates). A **Simulate Chest** button seeds a few random containers whose loot is rolled from the chest's own drop table, so you can try the window without real chests.
