...
**Seeing the route you're already on.** Once a route is *running*, a **Details…** button appears on the **CURRENT NAV** panel's header (in the right rail). It opens that same full step plan — in its own scrollable window — for whatever's currently executing: a point-to-point walk, a loop circuit, or an Auto-Lair approach, so you can check the path ahead without re-planning it. It adds one thing the picker's list doesn't: at every room on the route, its **notable monsters** — placed fixtures (a boss / NPC) and lair spawners — are listed right under that step, each a **clickable link** that opens the monster's Game Data record — handy for sizing up what a hunting loop is about to walk into. Click **Details…** again to close the window.
