...
| `@hydra dead` | — | records a hydra trainer kill report — who (you) and when — replacing whatever was recorded before; unlike `@timer`'s bosses this is never self-detected, only ever set by someone explicitly reporting it |
| `@hydra status` | — | who last reported the hydra dead, and how long ago (e.g. `hydra last reported dead by Alice, 2h14m ago`), or "no hydra kill reported yet" if nobody has; own permission ("Hydra timer") |
