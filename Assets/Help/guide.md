...
| `@roomba` | `<item name>` | every gang-house room that item was seen in during a Roomba sweep (one line each, with quantity and how long ago) — or "no record"; own permission ("Query item location"). Silent unless **Enable @roomba responses** is checked on the Roomba tab — see Roomba (Player Workshop) below |
