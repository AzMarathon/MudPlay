...
**Seeing the full step plan.** Whichever route fork the picker shows, once you **click a route** the **Show steps…** button lights up. It opens the complete, start-to-finish command sequence that route will execute — every move, and every **detour** (a lever pulled in another room, a winch cranked, a door opened) shown inline as `12/431 Tower < s`: the room you're standing in, then the command sent from it. Where a route must **acquire** an item to pass a gate (buy a raft, ask an NPC, hunt a drop), that shows as its own amber step marked with a **◆**, naming the room it detours from — e.g. `◆ 13/498 Sea Cavern — obtain a raft (buy at General Store)` — so you can see exactly which room step launches the fetch (the walk to the shop itself is planned when the run gets there, so it isn't listed hop-by-hop). It's the same expansion the walker runs, so what you read is what it will do. The button sits top-right and its list opens **to the side**, clear of the route cards. For a richer look, the **Details…** button at the picker's bottom-left opens the selected route in the same scrollable window described below — with each room's monsters, hazards, and item gates linked to their records.

**Seeing the route you're already on.** Once a route is *running*, a **Details…** button appears on the **CURRENT NAV** panel's header (in the right rail). It opens that same full step plan — in its own scrollable window — for whatever's currently executing: a point-to-point walk, a loop circuit, or an Auto-Lair approach, so you can check the path ahead without re-planning it. It also opens for a **previewed** walk-to — arm a destination in the search box (before you press Go) and Details… shows the route you're about to take. Three things the picker's plain list doesn't do:

- **Each room name is a link** — click it to flash the room on the map and centre there, the same as an `@where` reply.
- At every room on the route, its **notable monsters** — placed fixtures (a boss / NPC) and lair spawners — are listed under that step in **combat-red**, each a **clickable link** to the monster's Game Data record — handy for sizing up what a hunting loop is about to walk into.
