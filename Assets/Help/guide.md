...
**How long it stays in a lair.** It leaves for the next one as soon as the fight is over *and* the drops are picked up — it won't walk off and abandon loot it just fought for. "Fight over" means the room re-displays with no monster you'd engage left in it, which is the reliable signal; the game's own `*Combat Off*` line isn't usable on its own, since it also fires every time you cast and once per strike for thrown weapons and the like. **Engage timeout** (Settings → Auto-Lair, default 30s) is now only the upper bound, for a fight that never resolves — something you can't kill, one that ran away, or a lair you found empty. If Auto-Lair looks like it's idling in a lair, the usual reason is that there's nothing there to fight: check whether the monsters you're after actually spawn in that room, or wander into it from elsewhere on their own schedule.
