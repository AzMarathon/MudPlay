...
- **Chest Offload — pricing and selling** — each item has an editable sell quantity (keep some, sell the rest) priced at a **charm** picker, its own **Sell** button to sell just that item's quantity (while standing in its shop), and a **Drop** button to bin the whole stack. A **Total to sell** figure sums everything selected across all shops.
