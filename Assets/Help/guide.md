...
**`@where` on the map.** When you `@where` another MudPlay user and their client answers with its location (a telepath like `Fujin telepaths: {Adventurer's Guild, Universal Trainer (map 1, room 1376); exit s: west}`), the map — if it's open — briefly **flashes that room green and centres on it** for about 12 seconds, then drifts back to following you. It only reacts while the Navigation window is open; a reply that lands with the map closed is ignored. A fresh `@where` reply replaces the previous flash.
