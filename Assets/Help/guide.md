...
Every table tab has a **Columns ▾** button at its **top-right** — a picker to check/uncheck which columns show, so you can tailor each table to just the stats you care about. It also surfaces columns that are otherwise only used by the filter sidebar: on the Monsters tab, for instance, you can turn on the per-element resist columns (Cold / Fire / Stone / Lightning / Water), spell-immunity, and more, to *see* them in the grid instead of only filtering by them. Your choices are saved **per character**, per table; **Reset to defaults** in the picker restores that table's standard columns.
