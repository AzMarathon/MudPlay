...
| `@roomba` | `<item name>` | the gang-house room that item was last seen in during a Roomba sweep, its quantity, and how long ago — or "no record"; own permission ("Query item location"). Silent unless **Enable @roomba responses** is checked on the Roomba tab — see Roomba (Player Workshop) below |
...
**Item-location log + `@roomba`.** Every room floor Roomba observes during a scan (recon or the post-sort final scan) is recorded as that item's last-known location — not just what's currently sitting there, so the log stays useful between sweeps too. Tick **Enable @roomba responses** on the tab to let a gangpath'd `@roomba <item name>` (from a gang member you've granted the "Query item location" remote-control permission on the Players tab) get back the room it was last seen in, its quantity, and how long ago; left unticked, the client never replies to `@roomba` at all. Like the room labels, this log and the toggle are shared BBS-wide.
