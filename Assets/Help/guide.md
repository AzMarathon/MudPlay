...
Click a section to open it. Each table has its own **Filter…** box (this one filters *rows*), sortable and resizable columns, and a row-count line at the bottom. The rightmost **Use** column shows which tier owns each row — **Def** for the untouched import, or **Glob / BBS / Char** once you've overridden it. The **Monsters** table mirrors MegaMUD's Monsters tab, column-for-column: **Rgn** (respawn timer), **Exp** (the base reward with its multiplier, e.g. `65000 (20x)`), **HP**, **AC/DR**, **Dodge**, **MR**, **Acc (Maj/Mx)** (majority/highest attack accuracy), **Damage**, **Exp/(Dmg+HP)** (an exp-per-effort efficiency score), **Lair Exp**, **# Lairs**, **Mag** (the hitmag level a weapon must meet to land a hit), and **Undead**. It also carries a **filter sidebar** on the right, MegaMUD-style: one threshold per stat with the natural direction — **HP ≤**, **AC ≤**, **DR ≤**, **Dodge ≤**, **MR ≤**, **Acc ≤**, **Damage ≤**, **Mag ≤**, **Rgn ≤** for difficulty, and **Exp ≥**, **Lair Exp ≥**, **# Lairs ≥** for reward — plus an **Undead only** checkbox and an **Alignment** dropdown. Every filter stacks with the **Filter…** text box (all AND'd together); **Clear filters** resets them and the text box.
