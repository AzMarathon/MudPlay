...
The list also respects your **alignment** — a Good-only spell (or Evil-only, or Neutral-only) that your character couldn't actually learn is left out entirely, everywhere the class list feeds (the book itself and the Settings → Spells pickers alike). Alignment isn't part of `stat`'s output, so this only kicks in once the game has shown your own row in a `who` this session; before that, every alignment-gated spell still shows (nothing gets hidden on a guess).
