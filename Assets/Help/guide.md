...
**Add all blesses** adds a row for every self-castable buff you've learned in one click — no dialog, since there's nothing to choose. It looks at your spellbook for every buff you can cast on yourself (self-only spells, and single-target spells aimed at you) — **regardless of your current level**, so a buff you've outgrown or one whose level requirement you've already cleared both show — skips anything already slotted, and it's alignment-aware where it can be: a class's spellbook often carries both sides of a holy/unholy pair you've learned over your career (only one of which you can actually cast right now), and once MudPlay has seen your alignment in a `who` this run, only the side you can currently cast is offered.

It doesn't just check off one pick per buff family and hide the rest — **every** eligible buff gets a row, so you can see (and switch to) any of them. Where two buffs would strip each other off (e.g. **zeal** and **greater zeal**), only the higher-level one comes pre-checked; its family member is still listed, just unticked, so you can swap to it by hand. Checking one member of a conflicting pair automatically **unticks the other** — and vice versa — so you never end up with both fighting over the same slot; this live swap applies to any row in the list, not just ones this button added. It also leaves alone anything that conflicts with a buff you've already configured by hand (self or party-wide) — it lists it, but won't pre-check it, so it never silently clobbers your existing setup. It's self-only: a whole-party buff affects other players too, so those still go through **＋ Add buff** where you choose the targeting yourself. The button disables once there's nothing left to add.
