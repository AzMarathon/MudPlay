...
| `@roomba` | `<item name>` | the gang-house room that item was last seen in during a Roomba sweep, and how long ago — or "no record"; own permission ("Query item location"). Silent unless **Enable @roomba responses** is checked on the Roomba tab — see Roomba (Player Workshop) below |
