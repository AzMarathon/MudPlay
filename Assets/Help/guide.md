...
- **Items you auto-discard are left where they are.** If Auto-discard is set to throw an item away, Roomba won't collect it — otherwise the two would fight over it every lap (Roomba picks it up, Auto-discard bins it, Roomba finds it on the floor again next lap). They read the same discard list, so they can't disagree about what counts as junk. Items you keep *some* of are still sorted normally; only "discard all" items are skipped.
