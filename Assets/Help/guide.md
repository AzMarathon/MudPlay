...
**Avoiding traps on a walk-to.** If the shortest route to your destination crosses a trap and a **trap-free** route to the same room also exists, the route picker surfaces the choice: **"Avoid traps"** (the clean route, pre-selected) or **"Through the trap"** (the shorter shortcut, which the walker disarms at step time). The trap-free route is the default because a step-time disarm can fail (no lockpicks, no capable party member) and spring the trap — but the shortcut is one click away when you'd rather take it. Click either route to preview its line on the map, then **Go**. When there's **no** trap-free alternative, there's nothing to weigh, so the walk just proceeds and disarms en route as before.
