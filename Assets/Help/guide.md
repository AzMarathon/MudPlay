...
**In a party**, only the leader can take a token route. After the leader tokens across, it tells each member to use their own (`@do use <token>` over telepath — so each member must be carrying that town's token and must have granted you the *Execute commands* remote permission), then watches its landing room for each to gryphon in, retrying a few times a few seconds apart. Once everyone's regrouped the walk continues; if someone never makes it, the walk **fails out with the reason in the nav header** and sits so you can sort it out by hand. (A party follower who tries to take a token route just walks the normal way instead.)
