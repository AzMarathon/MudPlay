...
Each slot is a **row** with an **✎** (edit — reopens the dialog) and an **⨯** (remove) at the left, then the buff's `name (Lvl N) - recast` label — the level shown is the one the spell requires (i.e. was, or will be, learned at) — then the targeting checkboxes. **You choose who it's cast on right in the row:**
