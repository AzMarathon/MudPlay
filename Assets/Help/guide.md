...
- **Paradigm** prints "Uses remaining: N" when you `look` an item, so the count is read straight from that — the authoritative source. The client fills the readout in for you: any charged item you hold — **carried or worn** — whose count it doesn't know yet gets **looked automatically**, and it **re-looks after a use** (yours or one a party member remotes to you) to reconcile — so a use that's *blocked* (an NPC in the room, too little gold, too low a level) never wrongly drops the count. Transport tokens fill in the same way (they're looked on login).
