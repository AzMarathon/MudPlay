...
- **Sync Timers…** (on the Bosses tab) — share respawn timers with other MudPlay users. Pick a channel (Gang, Telepath to a named player, or Local say) and click **Request Timers**; the client sends `@timer sync` on that channel, and any other MudPlay user who receives it (and grants the `@timer` remote command) replies with their active timers, compressed onto a couple of chat lines. As each response arrives, the merge table fills in: one row per boss, your own timer beside a pickable button for each responder's. Click a button to choose whose timer to fold in for that boss (**Keep ours** is the default), then **Apply Selected** — only the rows you picked are written, so a stale or guessed timer from someone else never overwrites yours automatically. Bosses are matched by the monster itself (not by room), so it works even if you and they have pinned different rooms. A boss a responder tracks that **you don't** shows as its own row, and adopting it **adds the boss back to your list** — recovering a catalog boss you'd removed (e.g. to keep your room markers tidy) rather than orphaning the timer — so you can pick up a timer for a boss you weren't even tracking. Handy when someone caught an early spawn or has a real kill time where you were only guessing. You don't have to open the tab first: sending `@timer sync` by hand — a telepath (`/name @timer sync`) or a say (`.@timer sync`) — **auto-opens this merge window** and starts collecting, so the replies aren't lost when you fire the command from the terminal instead of the button.
