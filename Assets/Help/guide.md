...
- **Landmass**, **Region**, **Area** (right after the name) — where the monster lives, as the hierarchy Landmass → Region → Area (for example *Mainland → Volcano → Infernal Cavern*). The Paradigm set ships them filled in for every monster that has a spawn room or that something summons from a known room; the monster's most common place wins when it appears in several. Monsters with no spawn source anywhere in the game data (unused or test monsters, about 70) read blank. They are labels only — nothing in combat or navigation reads them — and they are searchable in the Filter… box. A monster with nothing set reads blank; fill it in from its record (below). If you've already customised this table's columns, the three start **unticked** — switch them on from **Columns ▾** (or **Reset to defaults**).
