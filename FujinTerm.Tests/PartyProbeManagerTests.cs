...
public sealed class PartyProbeManagerTests
{
    ...
    public void VersionReply_RecordedOntoPlayer()
    {
        ...
        h.Reply("Bob", "{FujinTerm 2.37.0}");

        Assert.Equal("FujinTerm 2.37.0", h.Players.Find("Bob")!.Version);
    }
    ...
    public void UnexpectedVersionLine_Ignored()
    {
        ...
        h.Reply("Carol", "{FujinTerm 2.37.0}");
    ...
    [Theory]
    [InlineData("{FujinTerm 2.37.0}", true, "FujinTerm 2.37.0")]
    [InlineData("{MegaMud 1.03u}", true, "MegaMud 1.03u")]
    [InlineData("FujinTerm 2.37.0", false, "")]          // not brace-wrapped
