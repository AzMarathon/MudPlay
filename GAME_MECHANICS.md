...
- **[CONFIRMED — Paradigm-specific]** **Paradigm lets a single action name a count** —
  `buy <N> <item>`, `sell <N> <item>`, `get <N> <item>`, `drop <N> <item>`, `give <N> <item>`,
  `hide <N> <item>` — where `<item>` is **anything in the Items table** of a game-data import: keys
  (`drop 3 black star key`), equipment, weapons, usable items, loot — and
  its confirmation echoes the count with the **SINGULAR** item name: e.g. `You hid 35 orc-head.`
  (not `orc-heads`). **Stock has no such batching — one item per action, for every verb and every
  kind of item** (the client emits both forms through `CountedCommand.Emit`). Consequence for inventory
...
- **[CONFIRMED] 2026-09-26, user** — **A worn item drops with a plain `drop <item>`** — no `rem`
  first; the game takes it off and drops it in one command. Client use: `@drop-all full` / Drop
  Everything drops worn gear with the same `drop` it uses for the pack (InventoryActionHandler).
