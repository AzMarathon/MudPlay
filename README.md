...
> - Both Messages seeds pre-fill `{null}` on the applied + wears-off slots of per-round attack/damage spells (energy cost 1–1000 with no lasting effect), so they drop off the Incomplete worklist; lasting-condition spells (poison/blind/hold/stun/…) are left untouched
