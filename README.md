...
- **Game data** — import MajorMUD `.MDB` databases, keep multiple game-data sets, and browse or override records across the 4-tier hierarchy in the Game Data Browser — whose **Monsters tab** is a full monster reference with a grouped, range-based filter panel: bracket HP / EXP / AC, filter by per-element resists (signed, to hunt vulnerabilities), spell-immunity level, magic-weapon requirement, type / alignment, "casts spells", "drops an item", and more. Every engine reads from this data. A dedicated **Monster Intel** window turns that reference into a fast pre-fight check — for your loaded character it shows each monster's chance to hit you and the rounds your weapon needs to kill it, so you can tell at a glance what's safe to fight.
