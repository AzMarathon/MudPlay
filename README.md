...
> - Consolidated per-item on-use message duplicates onto their cast spell's single record (auto-correcting ~38 `{spellname}`→`{target}` token/grammar variants), one set of messages per cast spell (paradigm −71, stock −26); a genuinely-different wording is left on the item record for review
> - A weapon **combat-proc** spell that only deals damage no longer flags as Incomplete — only a proc that applies a lasting effect does
