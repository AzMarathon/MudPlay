...
> - The estimate simulates an hour on the game's 5-second combat tick (720/hour): you bank exp only on a tick where you're on a live mob, and movement between lairs rides the downtime — a hop that fits before the next tick is free. Each lair respawns on its own clock; a dense loop stays pinned at the 720/hr tick cap (there's always one ready), while a loop that outruns a lair's respawn shows that pass arriving "early by Ns". NPC-placed fixtures (e.g. the 1/1765 slime beast) respawn instantly
