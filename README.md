...
> - The **Incomplete Messages** tab doubles as a fill-from-game worklist: alongside orphan records it surfaces any spell-linked message still missing a required line (caster / target / witness / applied / wears-off, and the fumble line on a Confused record), naming the gaps in a **Missing** column, with a leading **Spell #** column showing the linked spell number
...
> - Both Messages seeds pre-fill `{null}` on the applied + wears-off slots of every **instant** spell (zero duration — attack/damage, heal, cure, dispel, drain, summon), so they drop off the Incomplete worklist; a spell with **any duration** (buffs, and lasting debuffs like poison/confuse/hold/blind) keeps those slots, as do records already carrying an effect line or a condition flag
> - Message catalogues (spell + monster) fall back to the **bundled seed** when the Global seed is missing, so recognition never comes up empty because a seed failed to bootstrap — the per-set file and Global seed still win, so your edits are never overridden
...
> - On first launch after this update, the Messages catalogue is **force-reseeded once** from the shipped seed — your older Global seed + per-set message edits are backed up to `.bak` and replaced with the recurated one-record-per-spell data; re-apply local wording afterward (or recover from the `.bak`)
> - New **Upload edits** button on the Incomplete Messages tab (far right of Add / Remove): exports the message records you've changed/added vs the shipped seed — keyed by spell/item number, seed → new per field, plus a JSON block — to a timestamped Markdown file on your Desktop for folding back into the seed
> - An item's **on-use / proc message now lives on its cast spell's record** (resolved via the item's CastsSp ability), shared by every item that casts that spell — edit it from any one and all update; the item record lists its cast spell(s) as a clickable **Casts** link with the spell number. An item that casts nothing keeps its own item-anchored message
> - **weapon major bless** (paradigm #114) filled in: self line *"You raise your weapon into the air, summoning its power!"*, *"Your weapon is blessed!"* onset, *"The effects of the blessing wear off!"* wear-off; target + witness `{null}` (self-only)
> - Consolidated per-item on-use message duplicates onto their cast spell's single record — exactly **one** record per cast spell (the shared spell record wins; the duplicate item-linked record is dropped; ~38 `{spellname}`→`{target}` variants auto-corrected; paradigm adopts the item's corrected `{target}` on pure-bracketing clashes; stock keeps its authoritative V1.11p DB wording). A cast-less item's wield/remove event stays on the item
> - A weapon **combat-proc** spell (`%Spell` / on-swing) that only deals damage has its message record **deleted** (stock −27, paradigm −60) — it needs a message only when it applies a lasting effect (the darkwood staff's HoldPerson proc is kept); a bare command **on-use** cast (the nexus spear's slam) keeps its messages
> - Correlated the paradigm Messages seed against the latest Paramud MegaMUD export to back-fill buff onset / wear-off lines
