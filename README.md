...
> - Consolidated per-item on-use message duplicates onto their cast spell's single record (auto-correcting ~38 `{spellname}`→`{target}` token/grammar variants), one set of messages per cast spell; a genuinely-different wording is left on the item record for review. Paradigm adopts the item's corrected `{target}` where a clash was purely bracketing; stock keeps its authoritative V1.11p DB wording
