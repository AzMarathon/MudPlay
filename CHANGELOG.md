...
- Gear no longer strands you travelling and fighting in weak pre-rest gear — when the loop resumes moving after a rest, it now wears the While-Moving / Default set even if a rest gate still lingers (an HP rest finishing while mana was still below its target used to keep you in pre-rest gear for rooms on end, then into combat, getting ravaged); the pre-rest set is held only while you're actually sitting, and pre-rest swaps are also suppressed while in combat
