...
- **Manage Bosses** dialog to add / edit / remove entries — name, rooms, respawn type, exact-spawn, realm flags, a manual respawn-hours override for bosses game data can't resolve a timer for, and a "show in table" toggle to hide an entry from the tab (still tracked) — with its own filter; **Import / Export** share a realm's table as a JSON file. "Stop before" toggles inline in the table (walk-to halts one room short of a flagged boss room)
