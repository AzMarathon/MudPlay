...
- Buff timers: casting a buff that removes another (e.g. chant removes bless) no longer wrongly clears the caster's OWN timer when the two share a wear-off message — the wear-off is re-attributed to the clobbered victim (keyed off the last successful cast), so the survivor keeps counting
