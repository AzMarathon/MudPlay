...
- The estimate simulates an hour on the game's 5-second combat tick (720/hour): you bank exp only on a tick where you're on a live mob, and movement between lairs rides the downtime — a hop that fits before the next tick is free, so tight loops aren't penalised for travel. Each lair respawns on its own clock keyed to when you last killed it, so a fast lap that laps a slow lair before it repops counts that pass as a miss (NPC-placed fixtures like the 1/1765 slime beast respawn instantly and fire every pass); pool exp is the expected value across the lair's monster list
...
- Locked out while a loop is being built (both consume map clicks as waypoints); available again once the loop is running. "Load loop…" browses for a saved .loop file (defaults to the active set's Loops folder) to analyse it
