...
- A monster's ask-keyword teleports now show on its room: an **NPC transports** section in the Navigation Room info and the map tooltip (`ask Seher'Sahham activate → Damp Cavern, Wellspring (16/637) — 100,000 copper`), clickable to the destination
...
- Steps in order — add a BBS (Profile Management → Add → host/port → OK), add a character, import a game-data .mdb — only the ones you're missing show (plus a final Connect); Back/Next move manually
...
- Each filter is tightly anchored (gear swaps gated to known player names; the channel member list only right after its header) so a genuinely unknown spell/monster/proc line can never be hidden — that's still exactly what the queue surfaces
...
- Resting with a Pre-rest gear set that adds max HP/MA no longer thrashes: the rest holds until your Default-equivalent pool reaches the target, so reverting to your Default set doesn't strip the bonus and drop you straight back below the rest trigger (endless gear-swap/rest loop)
- Auto-walker no longer rams a hidden exit while dead-reckoning (blind or dark): it was trusting a stale "obvious exits" set from the last room it actually saw and skipping the required search; it now searches, and the reveal is confirmed even while blind
- Conversation/transaction log Flush and Close now fully drain the background writer, so nothing can read a log mid-write (follow-up to the off-thread log write)
- bug reports addressed: paradigm-20260921-074300, paradigm-20260921-084019, paradigm-20260921-114318, paradigm-20260921-145800
...
- bug reports addressed: paradigm-20260918-190830
...
- bug reports addressed: paradigm-20260918-204906
...
- bug reports addressed: paradigm-20260917-233549
...
- bug reports addressed: paradigm-20260915-211744
...
- bug reports addressed: paradigm-20260915-130624
...
- bug reports addressed: paradigm-20260915-061646, paradigm-20260915-061734, paradigm-20260915-124359
...
- bug reports addressed: paradigm-20260914-055853
...
- bug reports addressed: stock-20260914-000112, stock-20260914-000155, stock-20260914-003246
...
- The Navigation window's Loop button now pre-loads the last loop run this session into the builder, ready to Run again or re-Save; new Settings → General "Load last ran loop" toggle (on by default) turns this off if you'd rather always start empty
...
- Auto-deposit bank runs now **walk to and from** a gated grind area (key-door, hidden exit, summon-drop key, lever/ask-NPC gate) instead of going idle at the bank — the return leg uses the full gate-aware pathfinder, not a plain walk (walking out is easy; walking back in needed it)
...
- Tooltips are complete and class-aware, showing each effect's current value + magnitude: Health shows max HP (gain per point) + HP regen (idle/resting); carry weight shows current capacity + per-point rate (steeper past 100); casters show mana regen AND spellcasting under their actual casting stat (Mage INT, Priest WIL, Druid INT+WIL, Bard CHM — max mana is level×magery, not a stat); accuracy follows the Stock/Paradigm weighting
...
- **Sysop map** records the power (the client will use `sys map` to help locate in a follow-up)
