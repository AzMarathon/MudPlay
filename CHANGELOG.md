...
- The estimate simulates an hour on the game's 5-second combat tick (720/hour): you bank exp only on a tick where you're on a live mob, and movement between lairs rides the downtime — a hop that fits before the next tick is free, so tight loops aren't penalised for travel. Each lair respawns on its own clock; a dense loop with enough lairs stays pinned at the 720/hr tick cap (there's always one ready), while a loop that laps a lair before it repops shows that pass as arriving "early by Ns". NPC-placed fixtures (e.g. the 1/1765 slime beast) respawn instantly; pool exp is the expected value across the lair's monster list
- Loading a loop into the estimator centres the map on its first room, and the Save / Discard / Load / Clear buttons sit at the top of the panel
