...
- Consolidated per-item on-use message duplicates onto their cast spell's single record — merged the non-clashing (auto-correcting ~38 `{spellname}`→`{target}` token / grammar variants), so each cast spell carries exactly one set of messages (paradigm −71, stock −26 records); a genuinely-different wording is left on the item record for review rather than silently dropped, and an item-specific perspective line is never pulled into the shared record
- A weapon **combat-proc** spell that only deals damage no longer flags as Incomplete — its target / witness are `{null}` (a proc flags only when it applies a lasting effect, which its duration already gates)
