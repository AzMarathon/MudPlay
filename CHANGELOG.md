...
- Consolidated per-item on-use message duplicates onto their cast spell's single record — merged the non-clashing (auto-correcting ~38 `{spellname}`→`{target}` token / grammar variants), so each cast spell carries exactly one set of messages; a genuinely-different wording is left on the item record for review rather than silently dropped, and an item-specific perspective line is never pulled into the shared record. Paradigm additionally adopts the item's corrected `{target}` wording where a clash was purely a bracketed-placeholder difference; stock keeps its authoritative V1.11p spell-message-DB wording
