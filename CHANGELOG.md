...
- Buff timers: fixed two shared-message mix-ups between buffs that remove each other and share their cast / wear-off messages (e.g. bless removes chant on Paradigm). Casting one now refreshes ITS OWN timer (keyed off the buff we sent, not the shared "you feel lucky" line — it was refreshing the other buff and leaving the cast one "not up"), and the shared "wear off" that follows a clobbering cast no longer clears anyone — the survivor keeps counting and the clobbered buff stays up to render as "conflict"
