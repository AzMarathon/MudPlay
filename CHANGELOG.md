...
- Both Messages seeds pre-fill `{null}` on the applied + wears-off slots of per-round attack/damage spells (energy cost 1–1000, no lasting effect) — 255 paradigm / 202 stock records — so those pure-damage spells drop off the Incomplete worklist; spells that inflict a lasting condition (energy cost 0, or already carrying an effect line — poison/blind/hold/stun/…) are left untouched
