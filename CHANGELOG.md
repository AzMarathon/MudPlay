...
- Roomba: `@roomba sync` now ends with a `Sync Complete` marker line so the requester can see the whole reply landed; and a sync line dropped for a missing grant is now logged (at debug) instead of vanishing silently
