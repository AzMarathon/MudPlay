...
- **Open-window menus / hotkeys / toolbar buttons raise-to-front.** Every command that spawns a window (View menu entries, Tools panels, the InfoDialogs under Help, the toolbar icons that mirror them) **brings the existing window to the front if it's already open** rather than opening a duplicate or closing it. Same applies to the hotkeys (F2 / F4 / F9 / etc.) and the toolbar icon buttons that mirror those commands. This replaced the old toggle-to-close convention — re-selecting a buried window (behind another window, or another running client) closed the very thing the user was hunting for, so a fresh-install user with two clients open thought Profile Management never opened. Implementation pattern: track the open instance in a field or a `Dictionary<id, Window>`, hook `Closed` to null/remove the tracker, and on re-press call `RaiseExisting(existing)` (un-minimize + `Activate()`) instead of constructing a new window or closing the old one. A deep-link re-press (jump to a section / record) applies the navigation first, then raises. Closing is done through the window's own X / Cancel / Save controls. New windows follow the same pattern.
