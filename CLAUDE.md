...
- **Versioning (semver, post-1.0)**: the version lives once in `FujinTerm.csproj` `<Version>` (`AppInfo.Version` reads it back for the `@version` reply). Bump it by **change type, not PR size**:
  ...
  - **MINOR** — a **brand-new feature** (a capability that didn't exist before). One bump per new feature, and it resets PATCH to 0 (e.g. 1.5.11 + a new feature → 1.6.0).
  - **PATCH** — bug fixes **and enhancements to existing features** (a changed default, a tweak or improvement to something that already exists). **Increments by the number of bug reports handled plus the number of enhancements** — one report or one enhancement on 1.5.11 lands 1.5.12; a batch of five handed over together advances 1.5.11 → 1.5.16. An enhancement counts exactly like a bug report: it moves the third number, never the minor.

  A PR that mixes types takes the highest (a brand-new feature plus fixes/enhancements is a MINOR; fixes plus enhancements with no new feature stays a PATCH). The version counts the reports + enhancements, but the changelog does **not** need one entry per item: a batch shares a single consolidated `## <final-version>` entry whose bullets cover all the changes.
