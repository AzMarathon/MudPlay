...
/// <summary>
/// IL-scans the compiled MudPlay assembly for every field marked with
...
public sealed class SingleWriterInvariantTests
{
    ...
    {
        // Anchor on a known MudPlay type to find the assembly file.
        ...
        foreach (TypeDefinition type in asm.MainModule.GetTypes())
        {
            foreach (FieldDefinition field in type.Fields)
            {
                CustomAttribute? attr = field.CustomAttributes
                    .FirstOrDefault(a => a.AttributeType.FullName == "MudPlay.Services.OwnerAttribute");
