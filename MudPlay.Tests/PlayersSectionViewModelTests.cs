...
public sealed class PlayersSectionViewModelTests
{
    ...
    public void NoProfileLoaded_ShowsEveryRow()
    {
        ...
        PlayerDatabase db = BuildDb("MudPlay WuzHere", "Raijin", "Helper");
    ...
    {
        // Profile name "MudPlay WuzHere" → given "MudPlay" → the MudPlay
        // observation row is hidden, the others remain.
        PlayerDatabase db = BuildDb("MudPlay WuzHere", "Raijin", "Helper");
        ProfileService profile = BuildProfileWithName("MudPlay WuzHere");
        PlayersSectionViewModel vm = new(db, dialogs: null, profile: profile);

        Assert.Equal(2, vm.AllRows.Count);
        Assert.DoesNotContain(vm.AllRows, r => r.Get("Given Name") == "MudPlay");
    ...
    public void ProfileLoaded_GivenOnly_FiltersByGivenName()
    {
        ...
        PlayerDatabase db = BuildDb("MudPlay Wuz", "Raijin");
        ProfileService profile = BuildProfileWithName("MudPlay");
        PlayersSectionViewModel vm = new(db, dialogs: null, profile: profile);
    ...
    public void FilterIsCaseInsensitive()
    {
        ...
        PlayerDatabase db = BuildDb("mudplay");
        ProfileService profile = BuildProfileWithName("MUDPLAY");
        PlayersSectionViewModel vm = new(db, dialogs: null, profile: profile);
    ...
    public void BlankProfileName_ShowsEveryRow()
    {
        ...
        PlayerDatabase db = BuildDb("MudPlay", "Raijin");
        ProfileService profile = new();
        profile.LoadBlank();
    ...
    {
        // Swap from MudPlay → Raijin via ProfileLoaded event:
        // MudPlay's row reappears, Raijin's row hides.
        PlayerDatabase db = BuildDb("MudPlay", "Raijin");
        ProfileService profile = BuildProfileWithName("MudPlay");
        PlayersSectionViewModel vm = new(db, dialogs: null, profile: profile);
        Assert.Single(vm.AllRows);
        ...
        Assert.Single(vm.AllRows);
        Assert.Equal("MudPlay", vm.AllRows[0].Get("Given Name"));
    }
    ...
    public void ProfileClosed_ShowsEveryRow()
    {
        ...
        profile.LoadBlank();    // closes "MudPlay" draft, opens new empty draft
