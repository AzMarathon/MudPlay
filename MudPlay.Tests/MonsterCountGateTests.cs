...
// override. Pins the exact behavior change: a room-spelled room's survivors are
// finished (stay engaged) below the floor only when the flag is on.
public sealed class MonsterCountGateTests
{
    [Theory]
    // count, min, max, killAll, roomSpelled, expectedStay
    ...
    [InlineData(2, 3, 20, true, false, false)]   // below floor, flag on but NOT room-spelled → move on
    [InlineData(2, 3, 20, true, true, true)]     // below floor, flag on + room-spelled → STAY (finish leftovers)
    ...
    public void WithinWindow_MatchesExpected(int count, int min, int max, bool killAll, bool roomSpelled, bool expected)
        => Assert.Equal(expected, MonsterCountGate.WithinWindow(count, min, max, killAll, roomSpelled));

    [Fact]
    public void WithinWindow_MisconfiguredMinAboveMax_FailsOpen()
        => Assert.True(MonsterCountGate.WithinWindow(1, 5, 3, killAllEngaged: false, roomSpelled: false));
}
