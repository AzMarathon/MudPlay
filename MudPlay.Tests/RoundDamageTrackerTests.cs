...
public sealed class RoundDamageTrackerTests
{
    ...
    {
        using Harness h = new();
        h.Feed("MudPlay hits a giant rat for 5 damage!");
    ...
    public void MultipleHits_Aggregate()
    {
        using Harness h = new();
        h.Feed("MudPlay hits a giant rat for 5 damage!");
        h.Feed("MudPlay hits a giant rat for 7 damage!");
        h.Feed("MudPlay hits a giant rat for 3 damage!");
        h.Feed("*Combat Off*");
    ...
    public void BurstOfSwings_OneTick_CollapsesToSingleRound()
    {
        ...
        using Harness h = new();
        h.Feed("MudPlay hits a giant rat for 4 damage!");
        h.Feed("MudPlay hits a giant rat for 6 damage!");
        h.Feed("MudPlay hits a giant rat for 7 damage!");
        h.Tracker.OnCombatTick();
    ...
    public void SuccessiveTicks_EachWindowIsOwnRound()
    {
        ...
        h.Feed("MudPlay hits a giant rat for 5 damage!");
        h.Tracker.OnCombatTick();
        h.Feed("MudPlay hits a giant rat for 8 damage!");
        h.Tracker.OnCombatTick();
    ...
    public void TwoSeparateCombats_TwoRoundsWithIncrementingNumbers()
    {
        ...
        // Fight 2
        h.Feed("MudPlay hits a goblin for 8 damage!");
        h.Feed("*Combat Off*");
    ...
    public void Recent_PreservesInsertionOrder()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            h.Feed($"MudPlay hits a goblin for {i + 1} damage!");
            h.Feed("*Combat Off*");
        }
    ...
    public void TraceWriter_Closed_WhenShouldWriteTraceReturnsFalse()
    {
        ...
        h.Feed("MudPlay hits a goblin for 1 damage!");
        h.Feed("*Combat Off*");
        Assert.Single(h.Completed);
    }
}
